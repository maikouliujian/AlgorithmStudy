...
public class _164_最大间距 {
    ...
    public static void main(String[] args) {
        ...
        int a = new _164_最大间距().maximumGap(arr);
    ...
    public int maximumGap(int[] nums) {
        ...
        //TODO 最终目的是为了保证在桶内部元素之间不会出现最大gap,这样我们只需要关注每个桶内的最大值和最小值
        ...
        for (int i = 0; i < buckets.length; i++) {
            if (buckets[i]!=null && preMax!=Integer.MAX_VALUE){
                //最大间距可能出现在当前桶的最小值和前一桶的最大值
                maxGap = Math.max(buckets[i].min - preMax,maxGap);
            }
            if (buckets[i]!=null){
                preMax = buckets[i].max;
                //最大间距可能出现在当前桶的最大值和最小值之间
